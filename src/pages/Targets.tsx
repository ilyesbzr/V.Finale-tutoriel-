...
import { BudgetData, PotentialData, TargetData, FamilyTargets, ProductivityTarget } from '../types';
...
function Targets(): JSX.Element {
  ...
  // State for potential
  const [potentialData, setPotentialData] = useState<PotentialData>({
  ...
  const [isPotentialLocked, setIsPotentialLocked] = useState<boolean>(false);
  const [lockedPotentialData, setLockedPotentialData] = useState<PotentialData | null>(null);
  ...
  const [selectedFamiliesMechanical, setSelectedFamiliesMechanical] = useState<string[]>([
    'tires', 'shockAbsorbers', 'wipers', 'brakePads', 'batteries', 'discs'
  ]);

  const [selectedFamiliesQuickService, setSelectedFamiliesQuickService] = useState<string[]>([
    'tires', 'shockAbsorbers', 'wipers', 'brakePads', 'batteries', 'discs'
  ]);

  const [selectedFamiliesBodywork, setSelectedFamiliesBodywork] = useState<string[]>([
    'windshields'
  ]);

  const [familyTargets, setFamilyTargets] = useState<FamilyTargets>({
    mechanical: {
  ...
  // Event handlers
  const handleBudgetChange = (field: keyof BudgetData, value: string): void => {
    setBudgetData(prev => ({
      ...prev,
  ...
  const handlePotentialChange = (dept: string, field: string, value: string): void => {
    setPotentialData(prev => ({
      ...prev,
      [dept]: {
        ...prev[dept as keyof PotentialData],
        [field]: Number(value) || 0
      }
    }));
  };

  const handleTargetChange = (dept: string, field: string, value: string, customHandler?: (dept: string, value: string) => void): void => {
    if (customHandler) {
  ...
  const handleFamilyTargetChange = (dept: string, family: string, value: string): void => {
    setFamilyTargets(prev => ({
      ...prev,
      [dept]: {
        ...prev[dept as keyof FamilyTargets],
        [family]: Number(value) || 0
      }
    }));
  };

  const handleShowHistory = (department: string): void => {
  ...
  const handleCANetChange = (dept: string, value: string): void => {
    const grossRevenue = Math.round(Number(value) / 0.7);
    setTargetData(prev => ({
      ...prev,
      [dept]: {
        ...prev[dept as keyof TargetData],
  ...
  const handleCAPerHECChange = (dept: string, value: string): void => {
    const newRevenue = Math.round(Number(value) * targetData[dept as keyof TargetData].hecHours);
    setTargetData(prev => ({
      ...prev,
      [dept]: {
        ...prev[dept as keyof TargetData],
  ...
  const handleCAPerHourChange = (dept: string, value: string): void => {
    const newRevenue = Math.round(Number(value) * targetData[dept as keyof TargetData].totalHours);
    setTargetData(prev => ({
      ...prev,
      [dept]: {
        ...prev[dept as keyof TargetData],
  ...
  const handleCAPerHTChange = (dept: string, value: string): void => {
    const newRevenue = Math.round(Number(value) * targetData[dept as keyof TargetData].totalHours);
    setTargetData(prev => ({
      ...prev,
      [dept]: {
  ...
  const handleLockPotential = (): void => {
  ...
  const handleResetPotential = (): void => {
    if (lockedPotentialData) {
  ...
  const targetFields: TargetFields = {
    ...
    caNet: { 
      label: 'CA APV', 
      format: 'currency',
      getValue: (data: any) => Math.round(data.revenue * 0.7),
      onChange: handleCANetChange
    },
    caPerHT: { 
      label: 'CA APV/HT', 
      format: 'currency',
      getValue: (data: any) => data.totalHours ? Math.round(data.revenue / data.totalHours) : 0,
