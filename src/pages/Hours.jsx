...
export default function Hours() {
  ...
  const [activeTab, setActiveTab] = useState('avancement'); // 'avancement' ou 'objectifs'
  const [selectedPeriod, setSelectedPeriod] = useState('previous'); // 'previous', 'current', 'monthly'
  ...
  const handleShowDossiers = (item) => {
    // Générer des noms de clients français réalistes
    const generateClientNames = (count) => {
        ...
        <div className="space-y-6">
          ...
          <Card>
            <div className="px-4 py-3 border-b border-gray-200 flex justify-between items-center">
              <h3 className="text-xl font-semibold text-gray-900">Avancement des heures facturées</h3>
            </div>
            <CardContent>
              <div className="space-y-6">
                {billingProgressData.map((item) => (
                  <div key={item.name} className="metric-card-modern p-6">
                    ...
                    <div className="flex justify-between items-center mb-2">
                      <span className={`text-xl font-medium ${getProgressTextColor(item.percentage)}`}>
                        {t('common.projection')} : {item.projection}
                      </span>
