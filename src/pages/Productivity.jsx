...
function Productivity() {
  ...
  useEffect(() => {
    ...
    const transformedMechanics = planningData.departments.flatMap(dept => {
  ...
  const serviceResults = Object.entries(mechanicsByService).map(([service, serviceMechanics]) => {
  ...
  const handleDaysChange = (mechanicId, newDays) => {
  ...
  const handleJobTypeChange = (mechanicId, newJobType) => {
    setMechanics(prevMechanics =>
      prevMechanics.map(mechanic => {
        if (mechanic.id === mechanicId) {
          return {
  ...
  const handleTargetChange = (mechanicId, newTarget) => {
    setMechanics(prevMechanics =>
      prevMechanics.map(mechanic => {
        if (mechanic.id === mechanicId) {
          return {
  ...
  const handleNameChange = (mechanicId, newName) => {
    setMechanics(prevMechanics =>
      prevMechanics.map(mechanic => {
        if (mechanic.id === mechanicId) {
          return {
  ...
  const toggleNameEdit = (mechanicId) => {
    if (!isEditing) return;
  ...
  const getProductivityTextColor = (percentage) => {
  ...
  const calculateProratedProductivity = (billedHours, target) => {
  ...
  const renderServiceSection = (service, serviceMechanics) => (
    <React.Fragment>
      <tr>
        <td
          colSpan={2} 
          className={`px-6 py-3 text-xl font-medium ${SERVICE_HEADER_COLORS[service]} sticky left-0 z-10`}
        >
          {service}
        </td>
      ...
      {serviceMechanics.map((mechanic) => {
        ...
        return (
          <tr key={mechanic.id} className={`${SERVICE_COLORS[service]} hover:bg-gray-50`}>
            <td className={`px-6 py-4 whitespace-nowrap text-base font-medium text-gray-900 sticky left-0 z-10 ${SERVICE_COLORS[service]} border-r border-gray-200`}>
            ...
            <td className="px-6 py-4 whitespace-nowrap text-xl text-center text-gray-500">
              {isEditing ? (
                <select
            ...
            <td className="px-6 py-4 whitespace-nowrap text-sm text-center">
              <div className="flex flex-col items-center">
                <span className={`px-3 py-2 rounded-full text-xl font-bold ${
      ...
      {serviceMechanics.length > 0 && (
        <tr className={`${SERVICE_HEADER_COLORS[service]} bg-opacity-50`}>
          <td colSpan={2} className="px-6 py-2 whitespace-nowrap text-lg font-medium sticky left-0 z-10 border-r border-gray-200">
            <span className="text-xl font-medium">Total {service}</span>
          </td>
          <td className="px-6 py-2 whitespace-nowrap text-xl text-center font-medium">
                ...
                <thead className="bg-gray-50 sticky top-0 z-10">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-center text-lg font-medium text-gray-500 uppercase tracking-wider">
                      <span className="text-base">{t('common.target')}</span>
                    </th>
                    <th className="px-6 py-3 text-center text-lg font-medium text-gray-500 uppercase tracking-wider">
                      <span className="text-base">{t('common.actual')}</span>
                    </th>
                    <th className="px-6 py-3 text-center text-lg font-medium text-gray-500 uppercase tracking-wider">
                      <span className="text-base">Rentabilité</span>
                    </th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {serviceResults.map((result) => (
                    <tr key={result.service}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-xl text-center text-gray-900">
                ...
                <thead className="bg-gray-50 sticky top-0 z-20">
                  <tr>
                    <th className="px-3 sm:px-6 py-3 text-left text-lg font-medium text-gray-500 uppercase tracking-wider bg-gray-50 sticky left-0 z-30 border-r border-gray-200 min-w-[150px] sm:min-w-[200px]">
                      <span className="text-base">{t('productivity.name')}</span>
                    </th>
                    <th className="px-3 sm:px-6 py-3 text-center text-lg font-medium text-gray-500 uppercase tracking-wider">
                      <span className="text-base">{t('productivity.job')}</span>
                    </th>
                    <th className="px-3 sm:px-6 py-3 text-center text-lg font-medium text-gray-500 uppercase tracking-wider">
                      <span className="text-base">{t('productivity.daysPresent')}</span>
                    </th>
                    <th className="px-3 sm:px-6 py-3 text-center text-lg font-medium text-gray-500 uppercase tracking-wider">
                      <span className="text-base">{t('productivity.billingPotential')}</span>
                    </th>
                    <th className="px-3 sm:px-6 py-3 text-center text-lg font-medium text-gray-500 uppercase tracking-wider">
                      <span className="text-base">Potentiel de facturation au prorata</span>
                    </th>
                    <th className="px-3 sm:px-6 py-3 text-center text-lg font-medium text-gray-500 uppercase tracking-wider">
                      <span className="text-base">{t('productivity.billedHours')}</span>
                    </th>
                    <th className="px-3 sm:px-6 py-3 text-center text-lg font-medium text-gray-500 uppercase tracking-wider">
                      <span className="text-base">Rentabilité mensuelle</span>
                    </th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
...
function calculateProductivity(billedHours, target) {
  if (!target) return 0;
  return Math.round((billedHours / target) * 100);
}

function calculateBillingPotential(daysPresent) {
